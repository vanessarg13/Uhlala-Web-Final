...
function modalShow(event) {
    ...
    document.getElementById('1litro').innerHTML = `${prod[0].Presentación}  $${prod[0].Precio} `
    document.getElementById('5litros').innerHTML = `${prod[1].Presentación}  $${prod[1].Precio} `
    document.getElementById('10litros').innerHTML= `${prod[2].Presentación}  $${prod[2].Precio} `
    document.getElementById('text-granel').innerHTML= `Elegí la Cantidad  $${prod[3].Precio} `
    ...
    ajaxRequest.onreadystatechange = function(){
        ...
        if(ajaxRequest.readyState == 4){
            ...
            if(ajaxRequest.status == 200){
                ...
                const filtroCocina = productos.filter( i => i.Categoría === 'Cocina y Baño' || i.Categoría2 === 'Cocina y Baño' )
                const filtroCategoria = filtroCocina.filter( i => i.Subcategoría)
